...
This repository presents a safe reinforcement learning workflow in a controlled GridWorld environment. It combines tabular Q-learning, graph-based danger-map supervision, a neural safety classifier, and shield-guided action intervention.

The project evaluates how a learned safety shield changes reinforcement learning behaviour: whether it can reduce unsafe exploration while still allowing the agent to learn effective task policies.
...
```text
safe-rl-neural-safety-shield-gridworld/
├── assets/
...
Generated datasets, Q-tables, model checkpoints, full episode logs, and archive files are treated as reproducible local outputs. They are intentionally excluded from version control and can be regenerated by running the notebook.
...
* 100 grid cells.
* 15 hazard cells.
* 4 discrete actions: up, down, left, and right.
...
* next position.
* current distance to the nearest hazard.
* next-state distance to the nearest hazard.
...
* no shield.
* a lenient intervention threshold.
* a strict intervention threshold.
...
The repository includes selected visualisations and lightweight metric summaries for inspection.

| File                                             | Description                                           |
| ------------------------------------------------ | ----------------------------------------------------- |
| `assets/baseline_q_learning_comparison.png`      | Baseline Q-learning comparison plot                   |
| `assets/danger_map_heatmap.png`                  | Danger-map visualisation                              |
| `assets/safe_rl_threshold_comparison.png`        | Safe RL threshold comparison plot                     |
| `assets/safety_shield_confusion_matrix_test.png` | Test-set confusion matrix for the safety shield       |
| `assets/safety_shield_confusion_matrix_val.png`  | Validation-set confusion matrix for the safety shield |
| `assets/safety_shield_loss_curves.png`           | Safety shield training and validation loss curves     |
| `reports/baseline_metrics_summary_m0p1.csv`      | Baseline Q-learning metric summary                    |
| `reports/safe_rl_metrics_summary.csv`            | Safe RL threshold comparison metrics                  |
| `reports/safety_shield_loss_curves.csv`          | Safety shield loss-curve values                       |
| `reports/safety_shield_test_metrics.csv`         | Test-set metrics for the safety shield                |
| `reports/safety_shield_val_metrics.csv`          | Validation-set metrics for the safety shield          |

These files are generated experiment outputs. They are not substitutes for the full generated dataset, Q-tables, or model checkpoints.

## Files Not Included

The following generated files are intentionally excluded from this repository:
...
The repository does not include full generated datasets, trained model checkpoints, Q-tables, full episode logs, local cache files, or local export archives. These files can be regenerated by running the notebook.

## Reproducibility Notes

The notebook is designed to regenerate the full workflow from the implementation itself. A fresh run can recreate:

* the GridWorld environment.
* the danger map.
* the labelled state-action dataset.
* Q-learning baseline results.
* safety shield training results.
* Safe RL threshold-comparison results.
* saved metric tables and visualisations.

Results may vary slightly depending on random seeds, hardware, package versions, and runtime configuration.
...
More realistic Safe RL settings may involve larger state spaces, partial observability, delayed feedback, stochastic transitions, or sparse intervention signals.

The reported results should therefore be interpreted as a controlled demonstration of shield-guided reinforcement learning rather than a general-purpose safety guarantee for real-world deployment.
...
No open-source license is currently granted for this repository. The notebook, generated reports, visualisations, and documentation are provided for reference only unless otherwise stated.

Generated datasets, Q-tables, trained model checkpoints, full logs, and local export archives are not redistributed in this repository.
