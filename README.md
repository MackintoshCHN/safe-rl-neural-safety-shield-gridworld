...
This project implements a safe reinforcement learning workflow in a custom GridWorld environment. It combines tabular Q-learning, graph-based danger-map supervision, a neural safety classifier, and shield-guided action intervention.
...
2. **Baseline Q-learning**
   A tabular Q-learning agent is trained without a safety shield to show how an unconstrained RL agent can learn task-oriented behaviour while still entering unsafe states during exploration.
...
4. **Neural safety shield and Safe RL integration**
   A feed-forward neural network is trained to predict the risk class of each state-action pair. The trained shield is integrated into the Q-learning loop to intervene when a proposed action is predicted to be unsafe.
...
Generated datasets, Q-tables, model checkpoints, and full per-episode logs are treated as reproducible outputs. They can be regenerated by running the notebook.
...
The baseline experiment is used to evaluate learning progress, reward behaviour, path efficiency, and unsafe exploration before adding the safety shield.
...
Running the notebook can regenerate datasets, model files, Q-tables, logs, reports, and plots.

## Notes on Generated Files

The repository keeps the notebook, selected visualisations, and selected metric reports.
